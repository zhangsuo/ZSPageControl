...
@property (nonatomic, weak) id<ZSPageControlDataSource> dataSource;

@property (nonatomic, weak) id<ZSPageControlDelegate> delegate;

@property (nonatomic, strong) UIView *coverView;

@property (nonatomic, strong) UIImageView *coverImageView;

@property (nonatomic, strong) NSMutableArray<ZSPageControlDot *>* visibleDots;
...
- (instancetype)initWithDataSource:(id<ZSPageControlDataSource>)dataSource
                 ...
                 WithConfiguration:(void (^)(ZSPageControl *))configuration {
    if (self = [self init]) {
...
- (void)initDots {
    ...
    ZSPageControlDot *lastDot ;
    
    for (int i = 0; i < _numberOfPages; i++) {
        
        ZSPageControlDot *dot = nil;
        
        if (![self.dataSource respondsToSelector:@selector(pageControl:dotForIndex:)]) {
            dot = [[ZSPageControlDot alloc]init];
...
- (void)autoConfigBounds {
    ...
    [self.visibleDots enumerateObjectsUsingBlock:^(ZSPageControlDot * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)configDotsUI {
    ...
    [self.visibleDots enumerateObjectsUsingBlock:^(ZSPageControlDot * _Nonnull dot, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)pageControlDotTapAction:(UITapGestureRecognizer *)tapGes {
    ZSPageControlDot *dot = (ZSPageControlDot *)tapGes.view;
